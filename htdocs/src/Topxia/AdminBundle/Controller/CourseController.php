...
class CourseController extends BaseController
{
    ...
    public function newIndexAction(Request $request, $filter)
    {
        $currentUser = $this->getCurrentUser();
        if (isset($currentUser["schoolId"]) && $currentUser["schoolId"] == "") {
            unset($currentUser["school_id"]);
        }
        $conditions = $request->query->all();
        if ($filter == 'normal') {
            $conditions["parentId"] = 0;
        ...
        // 加上学校ID判断
        if (isset($conditions["school_id"]) && $conditions["school_id"] == "") {
        ...
        return $this->render('TopxiaAdminBundle:Course:newIndex.html.twig', array(
            'conditions'     => $conditions,
            'courses'        => $courses,
            'users'          => $users,
            'categories'     => $categories,
